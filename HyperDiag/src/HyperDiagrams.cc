...
void Kpi_diag(){
	...
	TString root=Form( "Hyp_Kpidiag_%03d.root", ItNum );
...
//	TPad* Pad_Draw;
	...
	string root   = Form( "Hyp_eeKdiag_%03d.root" , ItNum );
	string figure = Form( "Hyp_eeKdiag_%03d.pdf", ItNum );
	while( SearchFile(rootpath,root)!=0 ){
		ItNum++;
		root=Form( "Hyp_eeKdiag_%03d.root", ItNum );
	}
	figure = Form( "Hyp_eeKdiag_%03d.pdf" , ItNum );
	root   = Form( "Hyp_eeKdiag_%03d.root", ItNum );
/*
	while( SearchFile(outputpath,outputname,0)!=0 ){
		ItNum++;
		outputname=Form("TwlkAna_run%04d_%03d.root",runnum,ItNum);
 */
	figure = figurepath+figure;
...
//	Pad_Draw -> cd();
	...
	Cul = new TCurlyLine( CulPos[0][0], CulPos[0][1], CulPos[1][0], CulPos[1][1] );
	Cul -> SetWavy();
	MySetting->Setting_Line( Cul, 633, 3, 1, CulAmp, CulWL );
	...
	Lat->SetTextColor(633);                    	Lat->DrawLatex( GamPos[0]           , GamPos[1]        , "#it{#gamma}*"         );
...
//	Pad_Draw->Draw();
	...
	Ca->Print(figure.c_str(), "pdf");
	// Export as root file 
	ifp = new TFile( root.c_str(), "recreate" );
	ifp->cd();
	Ca->Write("Ca");
	ifp->Close();
...
void gamK_diag(){
	...
	string root   = Form( "Hyp_gamKdiag_%03d.root", ItNum );
	string figure = Form( "Hyp_gamKdiag_%03d.pdf" , ItNum );
	while( SearchFile(rootpath,root)!=0 ){
		ItNum++;
		root=Form( "Hyp_gamKdiag_%03d.root", ItNum );
	}
	figure = figurepath+figure;
	root   = rootpath+root;
/*
...
*/
	...
	h_Draw_ = new TH1D( "h_Draw_", "h_Draw_", 1, 0., 1. );
	MySetting->Setting_Hist1D( h_Draw_, "", "", "", 0, 0, 42, 0, 0 );
	h_Draw_->SetStats(kFALSE);
	h_Draw_->GetXaxis()->SetNdivisions(0);
	h_Draw_->GetYaxis()->SetNdivisions(0);
	h_Draw_->GetXaxis()->SetAxisColor(0);
	h_Draw_->GetYaxis()->SetAxisColor(0);
	h_Draw_->GetYaxis()->SetRangeUser(0., 1.1 );
	h_Draw_->Draw();

	Pt = new TPaveText( .040, 0.971, .510, 1.080 );
	MySetting->Setting_Pave( Pt, 52, 1, 22, 0.0654, 1, 1, 1, 422, 1001 );
	Pt->AddText("p(#gamma, K^{+})#Lambda reaction");
	...
	CulPos[1][0] = (Arc->GetX1())-(Arc->GetR1());
	CulPos[1][1] =  Arc->GetY1();
	CulPos[0][0] = ElPos[0][0];
	CulPos[0][1] = ElPos[1][0];

	//Gamma
//	GamPos[0] = CulPos[0][0]-0.025;//0.5*(CulPos[0][0]+CulPos[1][0]);
//	GamPos[1] = 0.5*(CulPos[0][1]+CulPos[1][1]);
	GamPos[0] = QuarkPos[0][0]-0.160;
	GamPos[1] = MyBox[2]->GetQuarkPosY(0)+0.100;
